...
export type ProjectConnectionWithProjectRequestQuoteMilestone =
  ProjectConnection & {
    project_request: ProjectRequest;
    quotes: (Quote & { milestones: Milestone[] })[];
  };
export type ProjectConnectionWithQuoteMilestone = ProjectConnection & {
  quotes: (Quote & { milestones: Milestone[] })[];
};
...
export const checkProjectConnectionByCollaborationStatus = (
  data:
    | ProjectConnectionWithQuoteMilestone
    | ProjectConnectionWithProjectRequestQuoteMilestone,
  status: ProjectConnectionCollaborationStatus,
) => {
  const now = new Date();
  // Pending quote: pending for biotech decision and not expired
  const pendingDecisionQuotes =
    data?.quotes?.filter(
      (q) =>
        q.expired_at &&
        now < q.expired_at &&
        q.status === QuoteStatus.PENDING_DECISION,
    ) || [];
  // Ongoing quote: accepted and has ongoing milestones
  const acceptedOngoingQuotes =
  ...
  if (status === ProjectConnectionCollaborationStatus.COMPLETED) {
    return (
      pendingDecisionQuotes.length === 0 &&
      acceptedOngoingQuotes.length === 0 &&
      acceptedCompletedQuotes.length > 0
    );
  }
  ...
  if (status === ProjectConnectionCollaborationStatus.ONGOING) {
    return (
      ...
      (pendingDecisionQuotes.length >= 0 &&
        acceptedCompletedQuotes.length === 0) ||
      (acceptedCompletedQuotes.length > 0 && pendingDecisionQuotes.length > 0)
    );
  }
  return false;
};
...
>(
  ...
  return data.filter((pc) =>
    checkProjectConnectionByCollaborationStatus(pc, status),
  ) as T;
};
