...
export const createMicrosoftEvent = async (
  client: Client,
  eventDetails: MicrosoftGraph.Event,
): Promise<MicrosoftGraph.Event> => {
  return await client.api('/me/events').post(eventDetails);
};

export const patchMicrosoftEvent = async (
  client: Client,
  eventDetails: MicrosoftGraph.Event,
): Promise<MicrosoftGraph.Event> => {
  return await client.api(`/me/events/${eventDetails.id}`).patch(eventDetails);
};
...
export const deleteMicrosoftEvent = async (
  ...
  return await client.api(`/me/events/${eventDetails.id}`).delete();
};
