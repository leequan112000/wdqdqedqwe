...
export const createBiotechInvoice = async (
  args: CreateBiotechInvoiceArgs,
  ...
  const biotechInvoiceItemInputs: Prisma.BiotechInvoiceItemUncheckedCreateWithoutBiotech_invoiceInput =
    {
      amount: currency(milestone.amount.toNumber(), { fromCents: true })
        .intValue,
  ...
  const emailData = receivers.map((r) => {
    const email = getUserEmail(r.user);
    return {
      emailData: {
