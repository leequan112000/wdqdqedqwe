...
export const createBlanketPurchaseOrder = async (
  args: CreateBlanketPurchaseOrderArgs,
  context: ServiceContext,
) => {
  const { po_number, name, amount, current_user_id } = args;
  const allowCreatePurchaseOrder = await hasPermission(
    current_user_id,
    CasbinObj.PURCHASE_ORDER,
    CasbinAct.WRITE,
  );
  ...
  invariant(
    !existingBPO,
    new PublicError('A Blanket PO with the same PO number already exists.'),
  );
...
export const calculateBlanketPurchaseOrderBalanceAmount = async (
  args: CalculateBlanketPurchaseOrderBalanceAmountArgs,
  context: ServiceContext,
) => {
  const { id } = args;

  const blanketPurchaseOrderTransactions =
    await context.prisma.blanketPurchaseOrderTransaction.findMany({
      where: {
        blanket_purchase_order_id: id,
      },
    });

  const remainingBalance = blanketPurchaseOrderTransactions.reduce(
    (balance, transaction) => {
      if (
        transaction.transaction_type ===
        BlanketPurchaseOrderTransactionType.CREDIT
      ) {
        return balance + transaction.amount.toNumber();
      }
      if (
        transaction.transaction_type ===
        BlanketPurchaseOrderTransactionType.DEBIT
      ) {
        return balance - transaction.amount.toNumber();
      }
      return balance;
...
export const updateBlanketPurchaseOrder = async (
  args: UpdateBlanketPurchaseOrderArgs,
  context: ServiceContext,
) => {
  const { id, po_number, name, amount, current_user_id } = args;
  const allowEditPurchaseOrder = await hasPermission(
    current_user_id,
    CasbinObj.PURCHASE_ORDER,
    CasbinAct.WRITE,
  );
  ...
  invariant(
    customer?.biotech_id === blanketPurchaseOrder.biotech_id,
    new PermissionDeniedError(),
  );

  if (blanketPurchaseOrder.blanket_purchase_order_transactions.length > 1) {
    invariant(
      toCent(amount) >= blanketPurchaseOrder.amount.toNumber(),
      new PublicError(
        'Amount adjustments are allowed only for increasing the amount when transactions are present, decreasing is not permitted.',
      ),
    );
    invariant(
      po_number === blanketPurchaseOrder.po_number,
      new PublicError(
        'Modification of the Purchase Order number is not allowed when transactions are associated with it',
      ),
    );
  ...
  const balanceAmount = await calculateBlanketPurchaseOrderBalanceAmount(
    { id },
    context,
  );

  const updatedBlanketPurchaseOrder =
    await context.prisma.blanketPurchaseOrder.update({
      where: {
        id,
      },
      data: {
  ...
  return {
    ...
    balance_amount: toDollar(
      updatedBlanketPurchaseOrder.balance_amount.toNumber(),
    ),
...
export const removeBlanketPurchaseOrder = async (
  args: RemoveBlanketPurchaseOrderArgs,
  context: ServiceContext,
) => {
  const { id, current_user_id } = args;
  const allowDeletePurchaseOrder = await hasPermission(
    current_user_id,
    CasbinObj.PURCHASE_ORDER,
    CasbinAct.DELETE,
  );
  ...
  invariant(
    blanketPurchaseOrder.blanket_purchase_order_transactions.length <= 1,
    new PublicError(
      'Unable to delete Blanket Purchase Order with associated transactions for record-keeping purposes.',
    ),
  );
  ...
  const removedBlanketPurchaseOrder =
    await context.prisma.blanketPurchaseOrder.delete({
      where: {
        id,
      },
    });
  ...
  return {
    ...
    balance_amount: toDollar(
      removedBlanketPurchaseOrder.balance_amount.toNumber(),
    ),
