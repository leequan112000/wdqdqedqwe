...
const resolvers: Resolvers<Context> = {
  ...
  Mutation: {
    verifyBiotechInvoicePayment: async (_, args, context) => {
      ...
      invariant(
        biotechInvoice.purchase_order ||
          biotechInvoice.blanket_purchase_order_transaction,
        new PublicError('This invoice is paid via ACH'),
      );
      invariant(
        biotechInvoice.payment_status !== InvoicePaymentStatus.PAID,
        new PublicError('Invoice has already paid'),
      );
      invariant(
        biotechInvoice.payment_status === InvoicePaymentStatus.PROCESSING,
        new PublicError(
          'Invoice payment status invalid to perform this action',
        ),
      );
      ...
      return await context.prisma.$transaction(async (trx) => {
        const { updatedBiotechInvoice } =
          await milestoneService.updateMilestoneAsPaid(
            {
              milestone_id: biotechInvoice.biotech_invoice_items[0]
