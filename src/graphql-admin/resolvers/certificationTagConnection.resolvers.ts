...
const resolvers: Resolvers<Context> = {
  Mutation: {
    createCertificationTagConnection: async (
      parent,
      args: { certification_tag_id: string; vendor_company_id: string },
      context,
    ) => {
      const { certification_tag_id, vendor_company_id } = args;

      const existingCertificationTagConnection =
        await context.prisma.certificationTagConnection.findFirst({
          where: {
      ...
      invariant(
        ...
        new PublicError('Certification Tag Connection already exist.'),
      );
      ...
      invariant(
        certificationTag,
        new PublicError('Certification tag not found.'),
      );
    ...
    deleteCertificationTagConnection: async (parent, args, context) => {
      ...
      invariant(
        existingCertificationTagConnection,
        new PublicError('Certification tag connection not found.'),
      );
