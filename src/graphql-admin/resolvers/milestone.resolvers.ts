  ...
  Mutation: {
    payVendor: async (_, args, context) => {
      ...
      invariant(
        ...
        new PublicError('Milestone has not been paid by the biotech yet'),
      );
      invariant(
      ...
      invariant(
        ...
        new PublicError('Vendor company has no Stripe account'),
      );
      ...
      try {
        ...
        invariant(
          transfer.id,
          new PublicError('Stripe Transfer: Missing transfer Id'),
        );
          ...
          await context.prisma.milestone.update({
            ...
            data: {
              vendor_payment_status:
                error.raw.code || MilestonePaymentStatus.UNPAID,
            },
          });
          throw new PublicError(
            `${error.raw.code} (Milestone #${milestone.id}) Transfer to connected account failed for ${vendorCompany.name}`,
          );
        }
