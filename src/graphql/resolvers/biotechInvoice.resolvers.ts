...
import {
  ...
  checkBiotechInvoicePermission,
} from '../../helper/accessControl';
import {
  AdminTeam,
  BIOTECH_INVOICE_ATTACHMENT_DOCUMENT_TYPE,
  BiotechInvoiceAttachmentDocumentType,
...
const resolvers: Resolvers<Context> = {
  BiotechInvoice: {
    ...
    blanket_purchase_order_transaction: async (parent, _, context) => {
      invariant(parent.id, 'Invoice ID not found.');
      const blanketPurchaseOrderTransaction =
        await context.prisma.blanketPurchaseOrderTransaction.findFirst({
          where: {
            biotech_invoice_id: parent.id,
          },
        });
      return blanketPurchaseOrderTransaction
        ? {
    ...
    biotech_invoice_attachment: async (parent, _, context) => {
      invariant(parent.id, 'Invoice ID not found.');
      const attachment =
        await context.prisma.biotechInvoiceAttachment.findFirst({
          where: {
            biotech_invoice_id: parent.id,
          },
        });
      ...
      return attachment
        ? { ...attachment, byte_size: Number(attachment.byte_size) }
        : {};
    },
  },
  Query: {
    biotechInvoices: async (_, args, context) => {
      ...
      if (has_paid !== undefined) {
        baseFilter.payment_status = has_paid
          ? InvoicePaymentStatus.PAID
          : { not: InvoicePaymentStatus.PAID };
      }
  ...
  Mutation: {
    uploadBiotechInvoicePaymentReceipt: async (_, args, context) => {
      ...
      const { filename, key, filesize, contentType } = await storeUpload(
        data,
        BIOTECH_INVOICE_ATTACHMENT_DOCUMENT_TYPE[
          BiotechInvoiceAttachmentDocumentType.PAYMENT_RECEIPT
        ],
      );
      ...
      const emailData = admins.map((admin) => ({
        emailData: {
          ...
          vendor_company_name: biotechInvoice.biotech_invoice_items[0].milestone
            ?.quote.project_connection.vendor_company.name as string,
