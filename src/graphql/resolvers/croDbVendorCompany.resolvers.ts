...
const resolvers: Resolvers<Context> = {
  CroDbVendorCompany: {
    vendor_company_subspecialties: async (parent, _, context) => {
      if (parent.vendor_company_subspecialties)
        return parent.vendor_company_subspecialties;
      invariant(parent.id, 'Missing vendor company id.');
      return await context.prismaCRODb.vendorCompany
        .findUnique({
          where: {
            id: parent.id,
          },
    ...
    vendor_company_locations: async (parent, _, context) => {
      if (parent.vendor_company_locations)
        return parent.vendor_company_locations;
      invariant(parent.id, 'Missing vendor company id.');
      return await context.prismaCRODb.vendorCompany
        .findUnique({
          where: {
    ...
    vendor_company_certifications: async (parent, _, context) => {
      if (parent.vendor_company_certifications)
        return parent.vendor_company_certifications;
      invariant(parent.id, 'Missing vendor company id.');
      return await context.prismaCRODb.vendorCompany
        .findUnique({
          where: {
  ...
  CroDbVendorCompanyType: {
    company_type: async (parent, _, context) => {
      ...
      if (!companyType) {
        companyType = (
          await context.prismaCRODb.vendorCompanyType.findUnique({
            where: {
              id: parent.id!,
            },
            select: {
