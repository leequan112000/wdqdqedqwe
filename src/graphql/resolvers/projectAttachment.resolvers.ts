...
import {
  bulkContractUpdateNoticeEmail,
  bulkContractUploadNoticeEmail,
  bulkDocumentUploadNoticeEmail,
} from '../../mailer';
...
async function getBuffer(
  stream: Readable,
): Promise<{ buffer: Buffer; byteSize: number }> {
  let byteSize = 0;
  const chunks: any[] = [];
    ...
    zoho_editor_url: async (parent, _, context) => {
      if (
        parent.document_type ===
          PROJECT_ATTACHMENT_DOCUMENT_TYPE[
            ProjectAttachmentDocumentType.REDLINE_FILE
          ] &&
        parent.filename
      ) {
        ...
        const result = await Promise.allSettled(
          files.map(async (f) => {
            ...
            if (uploadLimitTracker) {
              ...
              invariant(
                canUpload,
                new PublicError(`Not enought space - ${uploadData.filename}`),
              );
            ...
            if (uploadData.filename) {
              ...
              if (projectAttachment) {
                const versionedFilename = await getFilenameWithVersion(
                  fullFilename,
                  1,
            ...
            const { filename, key, filesize, contentType } = await storeUpload(
              uploadData,
              PROJECT_ATTACHMENT_DOCUMENT_TYPE[
                ProjectAttachmentDocumentType.FILE
              ],
            );
          ...
          if (r.status === 'fulfilled') {
            return {
              ...
              data: {
                ...
                document_type:
                  PROJECT_ATTACHMENT_DOCUMENT_TYPE[r.value.document_type],
              },
            };
          }
      ...
      return await context.prisma.$transaction(
        async (trx) => {
          ...
          if (user?.customer?.biotech_id) {
            ...
            await uploadLimitTracker.init(user.customer.biotech_id);
            uploadLimitTracker.deductUsed(
              byteToKB(existingContract?.byte_size || 0),
            );

            const { byteSize } = await getBuffer(stream);

            const canUpload = uploadLimitTracker.validateFilesize(
              byteToKB(byteSize),
            );
          ...
          return {
            ...
            data: {
              ...
              document_type:
                PROJECT_ATTACHMENT_DOCUMENT_TYPE[attachment.document_type],
            },
          };
        },
        {
    ...
    removeAttachment: async (parent, args, context) => {
      ...
      return await context.prisma.$transaction(async (trx) => {
        ...
        return {
          ...
          document_type:
            PROJECT_ATTACHMENT_DOCUMENT_TYPE[deletedAttachment.document_type],
        };
      });
    },
  },
